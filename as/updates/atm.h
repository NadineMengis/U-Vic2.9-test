...
!   mtrack        = maximum size for tracking arrays
      ...
      real at, elev, rh, tmsk, umsk, dn, de, fcor, solins
      real dnswro, dnswrl
      real fluxo, fluxl
      real uplwro, upsenso, upltnto
      real uplwrl, upsensl, upltntl 
      real outlwr, precip
      real evapo, evapl, disch, soilm, vflux
      real runoff, surf, rtbar, atbar, tbar, awx, awy, apress, sulph
      real co2dist, elev_sealev, flxadj, track_co2, track_sat

      real precipltnt
      
      common /embm_r/ precipltnt(imt,jmt)

      common /embm_r/ at(imt,jmt,2,nat), elev(imt,jmt)
#if defined O_ice_evp || defined O_embm_awind
      common /embm_r/ fluxl(imt,jmt,nat+2),fluxo(imt,jmt,nat+2)
     &              , rh(imt,jmt)
#else
      common /embm_r/ fluxl(imt,jmt,nat),fluxo(imt,jmt,nat)
     &              , rh(imt,jmt)
#endif
      common /embm_r/ tmsk(imt,jmt), umsk(imt,jmt), dn(imt,jmt,nat)
      common /embm_r/ de(imt,jmt,nat), fcor(imt,jmt), solins(imt,jmt)
      common /embm_r/ upltnto(imt,jmt),uplwro(imt,jmt),upsenso(imt,jmt)
      common /embm_r/ upltntl(imt,jmt),uplwrl(imt,jmt),upsensl(imt,jmt)      
      common /embm_r/ dnswro(imt,jmt), dnswrl(imt,jmt)
      common /embm_r/ outlwr(imt,jmt), precip(imt,jmt), vflux(imt,jmt)
      common /embm_r/ evapl(imt,jmt), evapo(imt,jmt), disch(imt,jmt)
...
!     ta_sulph      = time averaged sulph
      ...
      real ta_psno, ta_evap, ta_disch, ta_vflux, ta_ws, ta_soilm
      real ta_runoff, ta_surf, ta_wx, ta_wy, ta_awx, ta_awy, ta_rtbar
      real ta_apress, ta_flxadj, ta_dn, ta_de, ta_flux_co2, ta_co2emit
      real ta_sulph
      ...
      common /embm_r/ ta_vflux(imt,jmt), ta_ws(imt,jmt)
...
# endif
#if defined O_co2emit_data && defined O_carbon_co2_2d
...
# endif
#endif
      real fracl, fraci, fracbl
      common /embm_r/ fracl(imt,jmt), fraci(imt,jmt), fracbl(imt,jmt)
      real fraco, cir, fracblis
      common /embm_r/ fraco(imt,jmt), cir(imt,jmt), fracblis(imt,jmt)
      real aelev
      common /embm_r/ aelev(imt,jmt)            
#if defined global_subgrid           
      integer numE
      parameter (numE=21491)      
      real strtE, endE, resE, elevE, fracE
      common /embm_r/ strtE(imt,jmt), endE(imt,jmt), resE(imt,jmt)
      common /embm_r/ elevE(numE), fracE(numE)

      real prainE, sshE, psnowE, hsnowE, soilmE, mbalE, ticeE
      common /embm_r/ prainE(numE), sshE(numE), psnowE(numE)
      common /embm_r/ hsnowE(numE), soilmE(numE), mbalE(numE)
      common /embm_r/ ticeE(numE)
#endif 
      real resiE
      common /embm_r/ resiE(imt,jmt)
      integer strtiE, endiE, strtiEsav, endiEsav
      common /embm_i/ strtiE(imt,jmt), endiE(imt,jmt)      
      common /embm_i/ strtiEsav(imt,jmt), endiEsav (imt,jmt)      
      integer icemsk(imt,jmt),icegrid(imt,jmt)
      common /embm_i/icemsk,icegrid   
         
      real avgsno,avgmbal
      common /embm_r/ avgsno(imt,jmt),avgmbal(imt,jmt)   
#if defined O_topography_shading      
      real solinsiE, cosziE     
      common /embm_r/ cosziE(imt,jmt),solinsiE(imt,jmt)
#endif
