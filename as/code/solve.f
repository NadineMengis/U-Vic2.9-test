! source file: /net/mare/home1/eby/as/updates/solve.F
      subroutine solve (n)
...
!       temperature
     ...
     &      - dnswrl(i,j)*scatter !* (1.-tmsk(i,j))
     &      - dnswro(i,j)*scatter * tmsk(i,j)
     &      - outlwr(i,j)
     &      + uplwro(i,j) *tmsk(i,j) + uplwrl(i,j) !* (1.-tmsk(i,j))
     &      + upsenso(i,j)*tmsk(i,j) + upsensl(i,j))!* (1.-tmsk(i,j)))

!           latent heat from total precipitation as water
            forc(i,j) = forc(i,j) + precipltnt(i,j)*fb
!           correct for latent heat from snow
            forc(i,j) = forc(i,j) + fc*psnol(i,j)!*(1.-tmsk(i,j))
     &                            + fc*psnoo(i,j)!*tmsk(i,j)
     &                            + fc*nmbal_sno(i,j)!*(1.-tmsk(i,j))
!relic lines removed here.
          enddo
        enddo
...
!       humidity
        ...
        do j=2,jmtm1
          do i=2,imtm1
	    forc(i,j) = (fa*evapl(i,j))!*(1.-tmsk(i,j)))
     &	               +(fa*evapo(i,j)*(tmsk(i,j)))
          enddo
        enddo
