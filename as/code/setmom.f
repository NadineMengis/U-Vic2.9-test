! source file: /net/mare/home1/eby/as/updates/setmom.F
...
!           expecting units of degrees K
            ...
            if (exists) then
              ...
              call getvara ('temperature', iou, imtm2*km, ib, ic, tmpik
     &,         c1, c0)
              text = "C"
              call getatttext (iou, 'temperature', 'units', text)
!             convert to model units (C)
              if (trim(text).eq."K")
     &          where ((tmpik(1:imtm2,1:km)) .lt. 1.e30)
     &            tmpik(1:imtm2,1:km) = tmpik(1:imtm2,1:km) - C2K
              t(2:imtm1,1:km,j,itemp,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,itemp,taup1),imt,km)
            else
              if (jrow .eq. 1)
...
!             convert to model units ((psu-35)/1000)
              call getvara ('salinity', iou, imtm2*km, ib, ic, tmpik
     &,         p001, -p035)
              t(2:imtm1,1:km,j,isalt,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,isalt,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!           expecting units of mol m-3 (equals model units, umol cm-3)
            ...
            if (exists) then
              call openfile (trim(fname), iou)
              tmpik(1:imtm2,1:km) = t(2:imtm1,1:km,j,idic,taup1)
              call getvara ('tco2', iou, imtm2*km, ib, ic, tmpik
     &,         c1, c0)
              t(2:imtm1,1:km,j,idic,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,idic,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!           expecting units of mol m-3 (equals model units, umol cm-3)
            ...
            if (exists) then
              call openfile (trim(fname), iou)
              tmpik(1:imtm2,1:km) = t(2:imtm1,1:km,j,ialk,taup1)
              call getvara ('alk', iou, imtm2*km, ib, ic, tmpik
     &,         c1, c0)
              t(2:imtm1,1:km,j,ialk,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,ialk,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!           expecting units of mol m-3 (equals model units, umol cm-3)
            ...
            if (exists) then
              call openfile (trim(fname), iou)
              tmpik(1:imtm2,1:km) = t(2:imtm1,1:km,j,io2,taup1)
              call getvara ('oxygen', iou, imtm2*km, ib, ic, tmpik
     &,         c1, c0)
              t(2:imtm1,1:km,j,io2,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,io2,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!           expecting units of mol m-3
            ...
            if (exists) then
              call openfile (trim(fname), iou)
              tmpik(1:imtm2,1:km) = t(2:imtm1,1:km,j,ipo4,taup1)
!             convert to model units (nmol cm-3)
              call getvara ('phosphate', iou, imtm2*km, ib, ic, tmpik
     &,         c1e3, c0)
              t(2:imtm1,1:km,j,ipo4,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,ipo4,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!           expecting units of mol m-3
            ...
            if (exists) then
              call openfile (trim(fname), iou)
              tmpik(1:imtm2,1:km) = t(2:imtm1,1:km,j,ino3,taup1)
!             convert to model units (nmol cm-3)
              call getvara ('nitrate', iou, imtm2*km, ib, ic, tmpik
     &,         c1e3, c0)
              t(2:imtm1,1:km,j,ino3,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,ino3,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!           expecting units of permil
            ...
            if (exists) then
              call openfile (trim(fname), iou)
              tmpik(1:imtm2,1:km) = t(2:imtm1,1:km,j,ic14,taup1)
!             convert later
              call getvara ('bkgc14', iou, imtm2*km, ib, ic, tmpik
     &,         c1, c0)
              t(2:imtm1,1:km,j,ic14,taup1) = tmpik(1:imtm2,1:km)
              call setbcx (t(1:imt,1:km,j,ic14,taup1),imt,km)
            else
              if (jrow .eq. 1)
     &          print*, "Warning => Can not find ",trim(fname)
            endif
...
!       read reference density field if it exists
        fname = new_file_name ("slh_ref.nc")
        inquire (file=trim(fname), exist=exists)
        if (exists) then
