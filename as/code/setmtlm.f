! source file: /net/mare/home1/eby/as/updates/setmtlm.F
...
!-----------------------------------------------------------------------
      ...
      subroutine recalc_mtlm_points
      implicit none

      include "size.h"        !imt,jmt
      include "param.h"       !imtm1,jmtm1
      include "atm.h"         !tmsk,umsk
      include "mtlm.h"        !for adjusting land_map
      include "csbc.h"        !land_map
      include "pconst.h"      !epsln
	...
	do i=2,imtm1
	  if (kmt(i,j) .le. klmax) then
	    L = L + 1
            !requires initialization of the ice sheet first!
   	    if (icemsk(i,j) .eq. 0 .and. tmsk(i,j) .eq. 0.) then
	      land_map(i,j) = L
	      LAND_PTS = LAND_PTS + 1
