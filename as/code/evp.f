! source file: /net/mare/home1/eby/as/updates/evp.F
...
!=======================================================================
      ...
      real imsk(imt,jmt)  !Jer
      character(120) g_st

      imsk(:,:) = 0.0	      !Jer
      where (tmsk .gt. 0.0) imsk = 1.0
...
!           mask zeta (and therefore eta)
            zetan(i,j) = zetan(i,j)*imsk(i,j)
            zetae(i,j) = zetae(i,j)*imsk(i,j)
            zetas(i,j) = zetas(i,j)*imsk(i,j)
            zetaw(i,j) = zetaw(i,j)*imsk(i,j)
...
!     pressure P
     ...
     &                    *exp(-20.0*(1.0 - aiceocn(i,j,2)))      ! g/s^2
          enddo
        enddo
      enddo
