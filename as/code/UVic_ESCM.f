! source file: /net/mare/home1/eby/as/updates/UVic_ESCM.F
      program UVic_ESCM
...
!       P. Myers, R. Pacanowski, P. Poussart, S. Rahmstorf, R. Redler,
!       D. Robitaille, A. Rosati, M. Roth, C. Sabine, O. Saenko,
!       A. Schmittner, B. Semtner, W. Sijp, H. Simmons, A. Skvortsov,
!       R. Smith, P. Spence, D. Stone, C. Tricot, S. Valcke, A. Weaver,
...
!-----------------------------------------------------------------------
	  ...
	  if (loop .eq. ntspos) then
           !call to ice sheet here, after mom loop (which is where
           !timing is incremented) but before call to embm_rest,
           !(where nmbal and dicevol are written to embm restart)
	   !Ice is called at the end of the year, after a full year's
	   !full net mass balance has been generated.
 	    if (abs(relyr - nint(relyr)) .lt. 1.e-8) then
              call runism
 	    endif
          endif
          call momout (1, imt, 1, jmt)
...
!-----------------------------------------------------------------------
!     deallocate ice sheet pointer storage arrays
!-----------------------------------------------------------------------
      call deallocate_ice_pointers
...
!-----------------------------------------------------------------------
      ...
      call set (iscal, m, mapsbc(m), 's_calbl', m)
      call set (iscao, m, mapsbc(m), 's_calbo', m)
      ...
      call set (ibia, m, mapsbc(m), 'bia', m)
...
!-----------------------------------------------------------------------
     ...
     &,                 gamma1, gbio, epsbio, nuz, gamma2, nud0, LFe
     &,                 wd0, par, dtnpzd, redctn, ki, redptn, capr
     &,                 dcaco3, nupt0, redotn, jdiar
