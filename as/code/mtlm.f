! source file: /net/mare/home1/eby/as/updates/mtlm.F
...
! TB       = WORK Total burnt
! TAB      = WORK Total available to burn
! TE       = WORK Total emitted
! TA       = WORK Total area
! ARF      = WORK Area reduction fraction
! AB       = WORK Area burnt

      real LW_OUT_P(POINTS,NPFT), LW_OUT_S(POINTS), PAR(POINTS)
      real SWN_P(POINTS,NPFT), SWN_S(POINTS)
      real TB, TAB, TE, TA, ARF, AB
      ...
      do j=2,jmt-1
        do i=2,imt-1
          ...
          if (L .ne. 0) then
            ...
            sbc(i,j,iscal) = sbc(i,j,iscal)+(1.-ALBLAND(L))*TIMESTEP
...
!       diagnose the total land carbon before triffid
        do I=1,LAND_PTS
          L = LAND_INDEX(I)
          BURN(L) = CS(L) - RESP_S_DR(L)/GAMMA
          do N=1,NPFT
            BURN(L) = BURN(L) + FRAC(L,N)*C_VEG(L,N)
          ...
          do i=2,imt-1
            ...
            if (L .ne. 0) then
              ...
              do N=1,NPFT
               BURN(L) = BURN(L) - FRAC(L,N)*C_VEG(L,N)
              enddo
              sbc(i,j,iburn) = BURN(L)/segtim
              BURN(L) = BURN(L)/TIMESTEP/segtim
            endif
          enddo
        enddo
