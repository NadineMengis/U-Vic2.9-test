! source file: /net/mare/home1/eby/as/updates/iomngr.F
      subroutine ioinit
...
!         A unit opened, even without the knowledge of Getunit will
!         still be NAMED.  Getunit skips over it.

          open (i,FILE='/tmp/dummy',IOSTAT=iostat,status='unknown')
          if (iostat .eq. 0) then
            close (i)
            goto 101
          endif
...
!     "Normal" termination of the do loop indicates all units are in use
      ...
      print*, 'ERROR:  No i/o units available -- Execution terminated'
      call showunits
      stop '==>Getunit'
