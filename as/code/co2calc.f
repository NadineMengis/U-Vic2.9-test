! source file: /net/mare/home1/eby/as/2.9.old3/source/common/co2calc.F
      subroutine co2calc_SWS (t, s, dic_in, ta_in, co2_in, pt_in
     &,                       sit_in, atmpres, phlo, phhi, ph
     &,                       co2star, dco2star, pCO2surf, dpco2
     &,                       CO3surf, Omega_c, Omega_a)
...
!--------------------------------------------------------------------------
      ...
      real atmpres, dco2star, ph, pco2surf, dpco2, invtk, is, is2
      real k0, k1, k12, k2, kw, kb, ks, kf, k1p, k2p, k3p, ksi, C2K
      real ksp0c, ksp0a, casurf, co3surf, omega_c, omega_a
...
!     Should we be using K0 or ff for the solubility here?
      pCO2surf = co2star / ff
      dpCO2    = pCO2surf - co2*atmpres

!     Calculate [CO3] in mol/kg from co2star and hSWS
      CO3surf = k1*k2*co2star/hSWS2
...
!     Ksp0 for calcite:
      ...
      Ksp0c = exp(-395.8293 + (6537.773/tk) + 71.595*alog(tk)
     &      - 0.17959*tk + (-1.78938 + (410.64/tk)
     &      + 0.0065453*tk)*s**0.5 - 0.17755*s + 0.0094979*s15)

!     Ksp0 for aragonite:

      Ksp0a = exp(-395.9180 + (6685.079/tk) + 71.595*log(tk)
...
!     calcite (Omgc)
      Omega_c = Casurf*CO3surf/Ksp0c

!     aragonite (Omga)
      Omega_a = Casurf*CO3surf/Ksp0a
